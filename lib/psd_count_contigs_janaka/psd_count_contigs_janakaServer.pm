package psd_count_contigs_janaka::psd_count_contigs_janakaServer;
...
our %async_run_methods = (
        'count_contigs_async' => 'psd_count_contigs_janaka.count_contigs',
);

our %async_check_methods = (
        'count_contigs_check' => 'psd_count_contigs_janaka.count_contigs',
);
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'psd_count_contigs_janaka';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = psd_count_contigs_janaka::psd_count_contigs_janakaServerContext->new($self->{loggers}->{userlog},
...
{
    # Service psd_count_contigs_janaka requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for psd_count_contigs_janaka but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = psd_count_contigs_janaka::psd_count_contigs_janakaServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service psd_count_contigs_janaka.");
    }
...
package psd_count_contigs_janaka::psd_count_contigs_janakaServerContext;

use strict;

=head1 NAME

psd_count_contigs_janaka::psd_count_contigs_janakaServerContext
...
package psd_count_contigs_janaka::psd_count_contigs_janakaServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = psd_count_contigs_janaka::psd_count_contigs_janakaServer->new(
