...
my $config = new Config::Simple($config_file)->get_block('psd_count_contigs_janaka');
my $ws_url = $config->{"workspace-url"};
my $ws_name = undef;
my $ws_client = new Bio::KBase::workspace::Client($ws_url,token => $token);
my $auth_token = Bio::KBase::AuthToken->new(token => $token, ignore_authrc => 1);
my $ctx = LocalCallContext->new($token, $auth_token->user_id);
$psd_count_contigs_janaka::psd_count_contigs_janakaServer::CallContext = $ctx;
my $impl = new psd_count_contigs_janaka::psd_count_contigs_janakaImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_psd_count_contigs_janaka_' . $suffix;
